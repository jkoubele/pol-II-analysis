...
usage() {
    echo "Usage: $0 -i <input_folder> -o <output_folder> -a <aggregated_adapters_folder>"
...
while getopts ":i:o:a:d:l:L" opt; do
    case ${opt} in
        i )
            input_folder=$OPTARG
            ;;
...
if [ -z "$input_folder" ] || [ -z "$output_folder" ] || [ -z "$aggregated_adapters_folder" ]; then
...
for sub_folder in "$input_folder"/*; do
  ...
  if [ "$run_locally" = true ]; then
    ...
    sh "$repository_path"/scripts/trimming.sh -i "$sub_folder" -o "$output_folder"/"$sample_name" \
    -d "$docker_image_path" -a "$aggregated_adapters_folder"
  else
    echo "Submitting sample $sample_name"
    sbatch --output="$slurm_log_folder"/%j_%x.log --error="$slurm_log_folder"/%j_%x.err \
    "$repository_path"/scripts/trimming.sh -i "$sub_folder" -o "$output_folder"/"$sample_name" \
    -d "$docker_image_path" -a "$aggregated_adapters_folder"
  fi
done
