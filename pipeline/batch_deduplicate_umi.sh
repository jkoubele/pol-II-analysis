...
for sub_folder in "$input_folder"/*; do
  ...
  if [ "$run_locally" = true ]; then
    ...
    sh "$repository_path"/scripts/deduplicate_umi.sh -i "$sub_folder" -o "$output_folder"/"$sample_name" \
    -d "$docker_image_path"
  else
    echo "Submitting sample $sample_name"
    sbatch --output="$slurm_log_folder"/%j_%x.log --error="$slurm_log_folder"/%j_%x.err \
    "$repository_path"/scripts/deduplicate_umi.sh -i "$sub_folder" -o "$output_folder"/"$sample_name" \
    -d "$docker_image_path"
  fi
done
